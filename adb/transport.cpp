...
#if !ADB_HOST
    ALOGI("%s: starting read_transport thread on fd %d, SYNC online (%d)",
       t->serial, t->fd, t->sync_token + 1);
#endif
    p = get_apacket();
...
#if !ADB_HOST
            ALOGI("%s: remote read failed for transport", t->serial);
#endif
            put_apacket(p);
            break;
        }
...
#if !ADB_HOST
    ALOGI("%s: read_transport thread is exiting", t->serial);
#endif
    kick_transport(t);
    transport_unref(t);
}
...
static void write_transport_thread(void* _t) {
    ...
    D("%s: starting write_transport thread, reading from fd %d",
       t->serial, t->fd);
#if !ADB_HOST
		ALOGI("%s: starting write_transport thread, reading from fd %d",
       t->serial, t->fd);
#endif
...
#if !ADB_HOST
    ALOGI("%s: write_transport thread is exiting, fd %d", t->serial, t->fd);
#endif
    kick_transport(t);
    transport_unref(t);
}
