...
static void findFileForItem(std::vector<PacImg>& pacImgList, FileItem &fileItem) {
    for (const auto& pacImg : pacImgList) {
        if (pacImg.partition_name == fileItem.id) {
            ...
            fprintf(stdout, "\n %s,%s,%s,%s,%s,%s,%s,%s,%s,%llu", fileItem.backup.c_str(), fileItem.CheckCali.c_str(), fileItem.id.c_str(), fileItem.idAlias.c_str(), fileItem.type.c_str(), fileItem.flag.c_str(), fileItem.checkFlag.c_str(), fileItem.description.c_str(), fileItem.flashFile.c_str(), fileItem.fileSize);
            return;
        }
    }
}
