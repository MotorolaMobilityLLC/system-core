...
typedef void (*notify_function_pointer)(const char *tombstone_path, pid_t vm_pid,
    pid_t pid, pid_t tid, siginfo_t *siginfo, const std::map<pid_t, ThreadInfo>* thread_info);

static void notify_aed(const char* tombstone_path, pid_t vm_pid, pid_t pid,
    pid_t tid, siginfo_t *siginfo, const std::map<pid_t, ThreadInfo>* thread_info)
{
  void *handle = NULL;
  ...
  dlerror();
  notify_fptr = (notify_function_pointer)dlsym(handle, "crash_mini_dump_notify");
  if (notify_fptr == NULL) {
  ...
  notify_fptr(tombstone_path, vm_pid, pid, tid, siginfo, thread_info);
  dlclose(handle);
}
...
int main(int argc, char** argv) {
  ...
  if (need_notify_aee) {
    notify_aed(tombstone_path, vm_pid, target_process, g_target_thread, &siginfo, &thread_info);
  }
