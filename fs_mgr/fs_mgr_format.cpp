...
static int format_f2fs(char *fs_blkdev, bool crypt_footer)
{
    ...
    if ((ioctl(fd, BLKSSZGET, &sector_sz)) == -1) {
        PERROR << "Cannot get sector size";
        close(fd);
        return -1;
    }
    ...
    std::string size_str = std::to_string(dev_sz /sector_sz);

    const char* const args[] = {"/system/bin/make_f2fs",
                                                 "-d1",
                                                 "-f",
                                                 "-O","encrypt",
                                                 "-O","quota",
