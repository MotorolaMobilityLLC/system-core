...
//Determine whether it is the p412 version
static bool isP412(){
     LOG(INFO) << "RF2-SKU10 '" << base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU10") << "' RF2-SKU11 '" << base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU11") << "'";
     return base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU10") || base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU11");
}
...
//#ifdef MOTO_LATAM_FEATURE_4176
        ...
        std::string ro_software_sku = GetProperty("ro.boot.hardware.sku", "");
        ...
        if (base::EqualsIgnoreCase(ro_software_sku,"XT2231-4")) {
            ...
            properties["ro.product.model"] = "Lenovo K15 Plus";
            properties["ro.product.model_apac_lr"] = properties["ro.product.model"];
            properties["ro.product.model_apac_l"] = properties["ro.product.model"];
	        ...
	        if(isP412()){
                moto_product_suffix = "_l_emea";
            }
        } else if(base::EqualsIgnoreCase(ro_carrier,"retgb") || base::EqualsIgnoreCase(ro_carrier,"tescogb")
                    || base::EqualsIgnoreCase(ro_carrier,"oraeu") || base::EqualsIgnoreCase(ro_carrier,"windit") || base::EqualsIgnoreCase(ro_carrier,"timit")
                    || base::EqualsIgnoreCase(ro_carrier,"bouyfr") || base::EqualsIgnoreCase(ro_carrier,"o2gb")
                    || ((base::EqualsIgnoreCase(ro_carrier,"reteu") || base::EqualsIgnoreCase(ro_carrier,"playpl") || base::EqualsIgnoreCase(ro_carrier,"pluspl"))
                    && base::EqualsIgnoreCase(ro_compile_product,"hawaiip"))) {
            moto_product_suffix = "_emea_n";
        } else if((base::EqualsIgnoreCase(ro_carrier,"reteu") || base::EqualsIgnoreCase(ro_carrier,"playpl") || base::EqualsIgnoreCase(ro_carrier,"pluspl"))
                    && base::EqualsIgnoreCase(ro_compile_product,"hawaiipl")) {
            moto_product_suffix = "_emea";
	    if(isP412()){
                moto_product_suffix = "_l_emea_e";
            }
        } else if(base::EqualsIgnoreCase(ro_carrier,"retru")) {
            moto_product_suffix = "_ru";
        }else if(base::EqualsIgnoreCase(ro_carrier,"retjio") && isP412()) {
            moto_product_suffix = "_l_emea";
            if(base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU11")){
                moto_product_suffix = "_l_emea_q";
            }
        }

        //Init P412 ship date and product model name
        if(isP412()){     
            properties["ro.mot.product_wave"] = "2022.3";
	    ...
	    for (const auto& moto_overlay_prop : MOTO_OVERLAY_PROPS) {
		    ...
		    for (const auto& ro_props_target : RO_PROPS_TARGETS) {
                ...
                std::string carrier_moto_overlay_prop_target_model = base_prop + ro_props_target+ ".model";
                properties[carrier_moto_overlay_prop_target] = properties[carrier_moto_overlay_prop];
                if(isP412()){
                    if(base::EqualsIgnoreCase(GetProperty("ro.boot.software.sku", ""),"RF2-SKU11")){
                    properties["ro.product.model"] = "moto e32";  
                    } else {
                        properties["ro.product.model"] = "moto e22s"; 
                    }
                    properties["ro.product.model_apac_lr"] = properties["ro.product.model"];
                    properties["ro.product.model_apac_l"] = properties["ro.product.model"];
                    properties["ro.product.display"] = properties["ro.product.model"];
                    properties[carrier_moto_overlay_prop_target_model] = properties["ro.product.model"];
                }
		    }
                } else {
...
//#endif //MOTO_LATAM_FEATURE_4176
    ...
    // Update lmk psi params for 3G or 4G RAM
#ifndef RECOVERY
    android::meminfo::SysMemInfo smi;
    bool result = smi.ReadMemInfo();
    if (result) {
        uint64_t totalSize = smi.mem_total_kb();
        LOG(INFO) << "Device ram size : " << totalSize;
        std::string ram_type = "1g";
        ...
        std::string value = properties["persist.lmk.psi_partial_stall_ms_" + ram_type];
        if (!value.empty()) {
            properties["ro.lmk.psi_partial_stall_ms"] = value;
        }
        value = properties["persist.lmk.psi_complete_stall_ms_" + ram_type];
        if (!value.empty()) {
            properties["ro.lmk.psi_complete_stall_ms"] = value;
        }
    }
#endif
#endif
