...
static void import_kernel_nv(const std::string& key, const std::string& value, bool for_emulator) {
    ...
    if (key == "qemu") {
        ...
        property_set(android::base::StringPrintf("ro.boot.%s", key.c_str() + 12).c_str(),
                     value.c_str());
    }else if(key == "hardwareID") {//lukai.wt add for sim card detect
    	char hardwareID[32];
		strlcpy(hardwareID,value.c_str(),sizeof(hardwareID));
		if('0' == hardwareID[0]){
			property_set("ro.telephony.sim.count","1");
			property_set("persist.radio.multisim.config","ss");
		}else if('1' == hardwareID[0]){		
