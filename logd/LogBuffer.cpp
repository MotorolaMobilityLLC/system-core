...
int LogBuffer::log(log_id_t log_id, log_time realtime,
   ...
   clock_gettime(CLOCK_MONOTONIC, &ts_1);
    ...
    if (now_time > old_time) {
       if (line_count > log_detect_value) {
        buff = new char[1024];
        if (buff == NULL)
            goto log_much_exit;

        if (log_much_buf == NULL) {
            log_much_alloc_size = log_detect_value * EACH_LOG_SIZE;
        ...
        kernel_log_print("logd: android log much:line %d, %d, time %06d.\n",
            line_count, realtime.tv_sec, realtime.tv_nsec / 1000);
...
next_log:
        ...
        /* close(fd_file); */
        pthread_attr_t attr;
        if ((file_count / 8 > log_detect_value / 10) && !pthread_attr_init(&attr)) {
            struct sched_param param;

            memset(aee_string, 0, 50);
            kernel_log_print("logd:logmuch file total size %d.\n", log_much_used_size);
            sprintf(aee_string, "Android log much: %d, %d.", line_count, file_count);
