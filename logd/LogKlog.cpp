...
char nowtimestr[64] = {0}, lasttimestr[64] = "[]";
log_time lasttime = log_time(CLOCK_REALTIME);
...
void LogKlog::sniffTime(log_time &now,
    ...
    if (cp) {
        ...
        *buf = cp;
        ...
        strncpy(nowtimestr, lasttimestr, strlen(lasttimestr));
        memcpy(&now, &lasttime, sizeof(log_time));
        convertMonotonicToReal(now);
    }
}
...
twoWord:    while (--taglen && !isspace(*++cp) && (*cp != ':'));
    ...
    *np = '\0';
    ...
    int rc = logbuf->log(LOG_ID_KERNEL, now, uid, pid, tid, newstr,
                         (n + timelen <= USHRT_MAX) ? (unsigned short) (n + timelen) : USHRT_MAX);
    //  int rc = logbuf->log(LOG_ID_KERNEL, now, uid, pid, tid, newstr,
    //                     (unsigned short) n);
