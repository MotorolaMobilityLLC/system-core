...
static void *reinit_thread_start(void * /*obj*/) {
    ...
    while (reinit_running && !sem_wait(&reinit) && reinit_running) {
        ...
        if (value == true) {
            ...
            value = !strcmp(property, "eng");
            if (value == false) { /* user load  level is 0.5 eng load*/
               /* log_detect_value = log_detect_value / 2; */
               log_detect_value = 1000;
            }
            ...
            kernel_log_print("logd: logmuch detect, build type %s, detect value %s:%d:%d.\n",
                value ? "user":"eng", property, count, log_detect_value);
        } else {
            log_detect_value = 0;
